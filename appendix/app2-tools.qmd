...
library(tidyverse)
#library(readxl)
dat<- read_excel("example_data.xlsx", sheet = "main", na = "NA"
                 )

citations<- read.csv("app1-paper_selection/citations.csv")

citations_needed<- subset(citations, DOI %in% dat$DOI, 
                          select = c(DOI, Author, Publication.Year))

sample <- inner_join(citations_needed, dat, by = "DOI")


sample$RS_spatital_resolution_m <- as.numeric(sample$RS_spatital_resolution_m)
sample$RS_spectral_bands_no <- as.numeric(sample$RS_spectral_bands_no)

```

```{r}

ggplot(sample, aes(x = log(OA/(1-OA))))+
  geom_histogram()
```

```{r}
ggplot(sample, aes(x = log(RS_spectral_bands_no), y = log(OA/(1-OA)), 
                colour = model_group
                ))+
  geom_point()
```
...
library(GGally)
ggpairs(sample, c("OA","fraction_majority_class", "model_group", 
...
```{r}
ggplot(sample, aes(x = number_classes, y =log(OA/(1-OA))#, 
                #colour = file_name
                ))+
  geom_point()
 
ggplot(sample, aes(x = fraction_majority_class, y =log(OA/(1-OA))#, 
                #colour = file_name
                ))+
  geom_point()
ggplot(sample, aes(x = log(RS_spectral_bands_no), y =log(OA/(1-OA))#, 
...
sample <- rowid_to_column(sample, "ID") 

with_n <- which(!is.na(sample$total)
                & !is.na(sample$`confusion_matrix diagonal`))


sample_with_n <- sample[with_n, ]
sample_with_n <- sample_with_n[-21, ]
